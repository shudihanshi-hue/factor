...
!
! 帮助系统核心实现 — 中文翻译
! 原始文件: D:\factor\basis\help\help.factor
...
: ($navigation-links) ( topic -- )
    [
        [ prev-article [ 1array \ $long-link "上一篇:" ($navigation-link) ] when* ]
        [ next-article [ 1array \ $long-link "下一篇:" ($navigation-link) ] when* ]
        bi
...
: :help-debugger ( -- )
    ...
    ":r    - 错误时的保留栈" print
    ":c    - 错误时的调用栈" print
    ":edit - 跳转到源码位置（仅解析错误）" print

    ":get  ( var -- value ) 访问错误时刻的变量" print
    ":vars - 列出错误时刻的所有变量" print ;

: (:help) ( error -- )
    error-help [ help ] [ "无此错误的帮助文档。" print ] if*
    :help-debugger ;
